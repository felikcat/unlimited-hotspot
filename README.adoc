...
. *+* Absolute maximum performance with disregard for a wide range of OS support.
- Expection: jailbreaking or rooting is not required.
...
[%collapsible]
====
. https://pairvpn.com/install[Install PairVPN] from the App Store and on the clients (such as a Windows laptop).
...
.On AT&T / Cricket Wireless, HTTPS "packet chunking" is required to bypass throttling.

...
. *+* Allows hotspot/tethering on telecom plans that block it.
...
. *+* Within reason, grants you unlimited data. Depends on the telecom and your plan however.
...
. Install the following apps on the tethering (Android) device:
...
=== Routers
If the tethering device is plugged into a router, likely through USB, additional steps are required:
...
. Install the https://github.com/krlvm/PowerTunnel-Android#configuring[PowerTunnel for Android] app on the tethering device.
...
client/tethered to device with its LibertyTunnel addon enabled, and test again.
...
. Only if you have high ping or ping spiking issues: try disabling "Tethering hardware acceleration" in the Settings app.
